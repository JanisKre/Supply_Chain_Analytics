...
function computeNodeValues(nodes: string[], links: SankeyLink[]) {
  const inFlow = new Array(nodes.length).fill(0)
  const outFlow = new Array(nodes.length).fill(0)
  links.forEach((l) => { inFlow[l.target] += l.value; outFlow[l.source] += l.value })
  return nodes.map((_, i) => inFlow[i] > 0 ? inFlow[i] : outFlow[i])
}

// Returns the column level (0 = pure exporter, N = stage N importer) from the node name suffix.
function nodeLevel(name: string): number {
  const m = /(\d+)$/.exec(name)
  return m ? Number.parseInt(m[1]) : 0
}

function computeNodePositions(nodes: string[], links: SankeyLink[], numStages: number) {
  // x: level 0 → far left, level numStages → far right, middle levels spread evenly
  const nodeX = nodes.map((name) => {
    const level = nodeLevel(name)
    if (level === 0) return 0.01
    if (level === numStages) return 0.99
    const slots = Math.max(1, numStages - 1)
    return 0.28 + ((level - 1) / slots) * 0.44
  })
  ...
  const nodeFlow = computeNodeValues(nodes, links)
  const nodeY = new Array(nodes.length).fill(0.5)
  xBuckets.forEach((indices) => {
    const sorted = [...indices].sort((a, b) => nodeFlow[b] - nodeFlow[a])
    const n = sorted.length
    sorted.forEach((idx, pos) => { nodeY[idx] = n === 1 ? 0.5 : 0.05 + (pos / (n - 1)) * 0.9 })
  })
...
export default function Step4Page() {
  ...
  const numStages = Object.keys(stageOrder).length

  const nodeColors = visNodes.map((n) => {
    const level = nodeLevel(n)
    if (level === 0 || level === numStages) return '#6E6E73'
  ...
  const { nodeX, nodeY } = visNodes.length > 0
    ? computeNodePositions(visNodes, visLinks, numStages)
    : { nodeX: [], nodeY: [] }

  const nodeValues = useMemo(() => computeNodeValues(visNodes, visLinks), [visNodes, visLinks])
  const nodeLabels = useMemo(
    () => visNodes.map((n, i) => `${n}<br>${nodeValues[i].toLocaleString('en-US', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}`),
    [visNodes, nodeValues],
  )
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: 'var(--surface)', boxShadow: 'var(--shadow)' }}>
          <Plot
            ...
            layout={{
              ...
              height: Math.max(560, visNodes.length * 13),
            } as object}
